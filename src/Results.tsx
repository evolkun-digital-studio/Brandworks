...
// Duplicate the three cards so the coverflow can wrap without a visible gap.
...
function Header() {
  return (
    <>
      <h2 className="site-display mt-3 flex w-full max-w-[820px] flex-col items-center text-center sm:mt-4">
        <span>How Brands Perform</span>
        <span>With Brandworks</span>
      </h2>

      <p className="site-copy mt-4 w-full max-w-[700px] px-2 text-center text-neutral-600 sm:mt-5 sm:max-w-[760px] sm:px-0">
...
function CardContent({ result }: { result: Result }) {
  return (
    <>
      ...
      <div
        data-text
        className="absolute bottom-[clamp(18px,4vw,24px)] left-[clamp(18px,4vw,24px)] right-4 text-left will-change-opacity"
      >
        <div className="text-[clamp(19px,2.3vw,22px)] font-medium leading-[1.15] tracking-[-0.025em] text-white">
          {result.stat}
        </div>
        <div className="mt-1 text-[clamp(13px,1.5vw,15px)] font-normal leading-[1.4] text-white/90">
          {result.label}
        </div>
      </div>
    </>
...
function useCoverflow(
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      if (m.tier === 'mobile') {
        const width = Math.min(m.vw * 0.78, 360)
        const heightFromWidth = width / ratio
        m.h = Math.min(heightFromWidth, m.stageH * 0.84)
      } else if (m.tier === 'tablet') {
        const width = Math.min(m.vw * 0.42, 370)
        const heightFromWidth = width / ratio
        m.h = Math.min(heightFromWidth, m.stageH * 0.78, 445)
      } else if (m.tier === 'laptop') {
        m.h = Math.min(460, m.stageH * 0.77, m.vw * 0.42)
      } else if (m.tier === 'desktop') {
        m.h = Math.min(490, m.stageH * 0.8)
      } else {
        m.h = Math.min(520, m.stageH * 0.82)
      }

      m.h = Math.max(m.tier === 'mobile' ? 300 : 330, m.h)
      m.w = m.h * ratio
  ...
  return (
    <div
      ...
      className={
        mobileMode
          ? 'relative h-[230svh]'
          : 'relative h-[270vh] md:h-[280vh] lg:h-[310vh] xl:h-[330vh] 2xl:h-[350vh]'
      }
    >
      <section
        ref={sectionRef}
        aria-label="BrandWorks results"
        className="sticky top-0 h-svh min-h-[560px] overflow-hidden bg-white sm:min-h-[620px]"
      >
        <div
          data-header
          className="mx-auto flex w-full max-w-[1320px] flex-col items-center px-4 pt-[clamp(42px,7svh,86px)] text-center sm:px-6 md:px-8"
        >
          <Header />
        </div>
          ...
          <div className="absolute inset-0 [transform-style:preserve-3d]">
            {RING.map((result, index) => (
              <div
                ...
                className="absolute left-1/2 top-1/2 overflow-hidden rounded-[14px] [backface-visibility:hidden] [transform-style:preserve-3d] sm:rounded-[16px]"
                style={{
                  width: 'var(--cw)',
                  height: 'var(--ch)',
...
function ResultsStatic() {
  return (
    <section className="mx-auto flex w-full max-w-[1320px] flex-col items-center bg-white px-4 py-16 text-center sm:px-6 sm:py-20 lg:px-8 lg:py-24">
      <Header />

      <div className="mt-10 grid w-full grid-cols-1 gap-4 sm:mt-12 sm:grid-cols-2 sm:gap-5 lg:mt-14 lg:grid-cols-3 lg:gap-6">
        {results.map((result) => (
          <div
            key={result.label}
            className="relative mx-auto aspect-[413/480] w-full max-w-[413px] overflow-hidden rounded-[14px] sm:rounded-[16px]"
          >
            <CardContent result={result} />
          </div>
        ))}
